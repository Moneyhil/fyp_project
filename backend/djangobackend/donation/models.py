...
class Registration(models.Model):
    ...
    def set_otp(self, raw_otp):
        ...
        if self.last_otp_sent_at and (timezone.now() < self.last_otp_sent_at + datetime.timedelta(minutes=1)):
    ...
    def verify_otp(self, raw_otp, expiry_minutes=5):
        ...
        if timezone.now() > self.otp_created_at + datetime.timedelta(minutes=expiry_minutes):
